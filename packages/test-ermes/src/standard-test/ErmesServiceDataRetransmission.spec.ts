...
import { MessageType, TypeOfData } from "ermes-types";
 ...
 */
export function testErmesServiceDataRetransmission(
  ...
  let service1: IErmesService;
  let service2: IErmesService;
  let messageControl1: IErmesMessageControlService | undefined;
  let messageControl2: IErmesMessageControlService | undefined;
  let storage1: IErmesStorageAndCaching<MessageType> | undefined;
  let storage2: IErmesStorageAndCaching<MessageType> | undefined;
  ...
  describe('ErmesService Data Retransmission Tests', function () {
    ...
    describe('Storage Integration for Retransmission', function () {
      ...
      it('should store sent messages and retrieve them for retransmission', async function () {
        ...
        service2.onMessage((data: Uint8Array) => {
          receivedCount++;
        });
    ...
    describe('Periodic Missing Message Checks', function () {
      ...
      it('should start and stop periodic checks correctly', async function () {
        ...
        if (!(service1 as any).startMissingMessagesCheck) {
          this.skip();
        }

        const ermesService = service1 as any;
    ...
    describe('Threshold-Based Missing Message Control', function () {
      ...
      it('should only request missing messages when threshold is reached', async function () {
        ...
        if (!messageControl2 || !(service2 as any).checkAndRequestMissingMessages) {
          this.skip();
        }

        const ermesService = service2 as any;
    ...
    describe('Callbacks and Event Handling', function () {
      ...
      it('should trigger onDataSending callback before sending', async function () {
        ...
        let callbackData: TypeOfData | null = null;

        service1.onDataSending((data: TypeOfData) => {
      ...
      it('should trigger onDataSended callback after sending', async function () {
        ...
        service1.onDataSended((data: TypeOfData) => {
          callbackTriggered = true;
          callbackData = data;
        });
