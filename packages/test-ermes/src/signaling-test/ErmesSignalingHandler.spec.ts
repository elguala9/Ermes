...
import { IErmesSignalingHandler, IErmesSignalingServer, IdAccountType, SignalType, SocketDTO, SocketReadyCallback } from "iermes/index";
import chaiAsPromised from "chai-as-promised";
import * as chai from "chai";


export function testSignalingHandler<SocketType>(
  service_1: IErmesSignalingHandler<SocketType>,
  service_2: IErmesSignalingHandler<SocketType>,
  service_3: IErmesSignalingHandler<SocketType>,
  account_1: IdAccountType,
  account_2: IdAccountType,
  account_3: IdAccountType
) {
  ...
  describe('IErmesSignalingHandler Tests', function () {
    ...
    it('processSignal() should reject null/undefined signals', async () => {
      await expect(service_1.processSignal(null as any, account_2)).to.be.rejected;
      await expect(service_1.processSignal(undefined as any, account_2)).to.be.rejected;
    });
    ...
    it('onSocketReady() should trigger callback when socket becomes ready', async () => {
      ...
      let receivedSocket: SocketDTO<SocketType> | null = null;

      const callback: SocketReadyCallback<SocketDTO<SocketType>> = (socket) => {
    ...
    it('onSocketReady() should handle callback for non-existent peer', async () => {
      let callbackTriggered = false;
      const callback: SocketReadyCallback<SocketDTO<SocketType>> = () => {
        callbackTriggered = true;
      };
    ...
    it('onSocketReady() should work with multiple peer callbacks', async () => {
      ...
      const callback1: SocketReadyCallback<SocketDTO<SocketType>> = () => {
        callback1Triggered = true;
      };
      
      const callback2: SocketReadyCallback<SocketDTO<SocketType>> = () => {
