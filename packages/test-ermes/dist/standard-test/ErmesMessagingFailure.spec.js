...
class FailingRepositoryWrapper {
    ...
    getController() {
        return {
            setFailureRate: (rate) => { this.failureRate = Math.max(0, Math.min(1, rate)); },
            setDropNextMessages: (count) => { this.dropNextCount = count; },
            resetFailures: () => {
                this.failureRate = 0;
                this.dropNextCount = 0;
                this.droppedMessages = 0;
...
export function testErmesMessagingFailure(f) {
    let service_1;
    let service_2;
    let failureController_1;
    let failureController_2;
    let repositoryWrapper_1;
    let repositoryWrapper_2;
    describe('Ermes Messaging Failure Tests', function () {
        before(async function () {
            ...
            repositoryWrapper_1 = new FailingRepositoryWrapper(service_1._repository);
            repositoryWrapper_2 = new FailingRepositoryWrapper(service_2._repository);
        ...
        after(function () {
            if (repositoryWrapper_1)
                repositoryWrapper_1.restore();
            if (repositoryWrapper_2)
                repositoryWrapper_2.restore();
        });
        ...
        it('should handle random message failures with 50% failure rate', async function () {
            ...
            const testMessages = Array.from({ length: messageCount }, (_, i) => new Uint8Array([i + 1, i + 2, i + 3]));
        ...
        it('should handle large message failure during chunking', async function () {
            ...
            if (messageReceived && receivedData !== null) {
                ...
                expect(receivedData.length).to.equal(largeSize);
