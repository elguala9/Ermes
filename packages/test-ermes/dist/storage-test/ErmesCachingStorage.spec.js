...
export function testService(cachingService) {
    describe('Service Tests', () => {
        ...
        it('Massive insert and retrive', async () => {
            let n = 1000;
            let array = generateUniqueMessageData(n);
            const t0 = performance.now();
            await StoreAndRetrive(cachingService, array, eqMessageData);
            const t1 = performance.now();
            const total = t1 - t0;
            console.log(`✅ ${n} insert+retrieval in ${total.toFixed(3)} ms` +
...
//# sourceMappingURL=ErmesCachingStorage.spec.js.map