...
// Leggi il file come stringa
    ...
    let SDK_1 = await SignalingSdkFactoryNode(rpc, privateKeys[4], address);
    let SDK_2 = await SignalingSdkFactoryNode(rpc, privateKeys[5], address);
    let handler = new SignalManager();
    let factorySignaling = new ErmesSignalingFactory();
    let service_1 = factorySignaling.create(SDK_1, handler);
    let service_2 = factorySignaling.create(SDK_2, handler);
